...
//        B = 393.1,
        ...
        BACK_LEFT = 113.0,
        BACK_RIGHT = 113.0,
        MIDDLE = 134.0; //122 at practice field           
...
//        MAX_VOLT_TILT = SmartDashboard.getNumber("Tilt");
    ...
    public boolean autoTilt(){
       ...
       // System.out.println("Calibrated Pot. "+ getCalibratedValue());
        SmartDashboard.putNumber("Setpoint", getUnCalibratedSetPoint());
...
//        }
   ...
   public void rampUpFirst(){
       try{
           firstWheel.setVoltageRampRate(RAMP_UP_RATE);
           firstWheel.setX(FORWARD);
       } catch (CANTimeoutException ex) {ex.printStackTrace();}       
   }
   public void rampUpSecond(){
       try{
   ...
   public void turnOnFirst(){
       try{
           firstWheel.setVoltageRampRate(0.0);
           firstWheel.setX(FORWARD);
       } catch (CANTimeoutException ex) {ex.printStackTrace();}           
   }
   public void turnOnSecond(){
       try{
           secondWheel.setVoltageRampRate(0.0);
           secondWheel.setX(FORWARD);
       } catch (CANTimeoutException ex) {ex.printStackTrace();}           
   }
   public void firstWheelQuarter(){
       try{
           firstWheel.setVoltageRampRate(0.0);
           firstWheel.setX(FORWARD/4);
       } catch (CANTimeoutException ex) {ex.printStackTrace();}         
   }
   public void secondWheelQuarter(){
       try{
           secondWheel.setVoltageRampRate(0.0);
           secondWheel.setX(FORWARD/4);
       } catch (CANTimeoutException ex) {ex.printStackTrace();}         
   }   
   public void firstWheelHalf(){
       try{
           firstWheel.setVoltageRampRate(0.0);
           firstWheel.setX(FORWARD/2);
       } catch (CANTimeoutException ex) {ex.printStackTrace();}         
   }
   public void secondWheelHalf(){
       try{
           secondWheel.setVoltageRampRate(0.0);
           secondWheel.setX(FORWARD/2);
       } catch (CANTimeoutException ex) {ex.printStackTrace();}         
   }   
   public void disableRampUp(){
       try{
           firstWheel.setVoltageRampRate(0.0);
           secondWheel.setVoltageRampRate(0.0);
       } catch (CANTimeoutException ex) {ex.printStackTrace();}       
   ...
   public double getUnCalibratedSetPoint(){
    ...
    public void shooterOn(double modifier){
        ...
        SmartDashboard.putBoolean("Shooter", isOn);
        System.out.println("Shooter On!");
    }
    public void autoShooterOn(){
//        try{
//            firstWheel.setX(FORWARD);
//            secondWheel.setX(FORWARD);
//          } catch (CANTimeoutException ex) {ex.printStackTrace();} 
        firstWheel.set(FORWARD);
        secondWheel.set(FORWARD);
        isOn = true;
    }
